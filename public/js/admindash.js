...
async function addTest(event) {
  ...
  try {
    ...
    console.log('📤 Creating test:', { name, description })
    ...
    if (response.ok) {
      ...
      if (data.success) {
        showNotification('Test created and saved to database! ✅', 'success')
        ...
        showNotification('Error: ' + (data.message || 'Failed to create test'), 'error')
        closeModal()
        return
      }
    } else {
      ...
      showNotification('Server error: ' + (errorData.message || response.statusText), 'error')
      closeModal()
      return
    }
  } catch (error) {
...
function createQuestionsChart(textColor, gridColor) {
  ...
  console.log('Creating questions chart...')
  
  // Count questions by subject
  const subjects = ['physics', 'chemistry', 'maths', 'biology']
  const questionCounts = subjects.map(subject => 
    questions.filter(q => q.testId === subject).length
  )
  
  // If no data, show placeholder data
  const hasData = questionCounts.some(count => count > 0)
  const chartData = hasData ? questionCounts : [1, 1, 1, 1]
...
function createResultsChart(textColor, gridColor) {
  ...
  const hasData = avgScores.some(score => score > 0)
  const chartData = hasData ? avgScores : [0, 0, 0, 0]
  ...
  // Add empty state overlay if no data
  if (!hasData) {
    addEmptyStateOverlay(ctx.parentElement, 'No test results yet')
  }
}
