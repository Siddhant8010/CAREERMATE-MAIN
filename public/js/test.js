(function () {
  ...
  function renderIndicators() {
    const bySubject = { physics: [], chemistry: [], maths: [], biology: [] };
    ...
    function build(container, indices) {
      ...
      indices.forEach((i, idx) => {
        ...
        btn.dataset.questionIndex = i; // Store index for easy lookup
        btn.style.animation = `indicatorPop 0.3s ease-out ${idx * 0.05}s both`;
  ...
  function renderQuestion() {
    ...
    if (questionNumberEl) questionNumberEl.textContent = `Question ${currentIndex + 1}`;
    ...
    (q.options || []).forEach((opt, index) => {
      ...
      const handleAnswer = () => {
        answers[q._id] = opt;
        console.log(`✓ Answered question ${currentIndex + 1} (ID: ${q._id})`);
